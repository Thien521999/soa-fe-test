  ...
  return (
    <section>
      <div className="ct-container flex items-center justify-between w-full">
        <h3 className="text-[48px] text-acent font-semibold leading-[60px] uppercase">Titre</h3>
        <div className="absolu flex gap-1 text-[20px] font-medium leading-[30px] capitalize">
          En savoir plus
          <Image src="/icons/ArrowRight.svg" alt="ArrowRight" width="24" height="24" />
        </div>
      </div>
      ...
      >
        {data &&
          data?.payload[0]?.bloc_3?.cases?.map((item: any) => (
            <SwiperSlide key={item.category} className="flex justify-center items-center">
              <motion.div
